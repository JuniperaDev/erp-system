...
public class AutonomousReportAttachmentInterceptor {
    ...
    public AutonomousReportAttachmentInterceptor(ReportAttachmentService<AutonomousReportDTO> reportAttachmentService) {
    ...
    public ResponseEntity<AutonomousReportDTO> attachReportToResponse(ProceedingJoinPoint joinPoint) throws Throwable {
        Logger log = logger(joinPoint);
        if (log.isDebugEnabled()) {
            log.debug("Enter: {}() with argument[s] = {}", joinPoint.getSignature().getName(), Arrays.toString(joinPoint.getArgs()));
        }
        try {
            ResponseEntity<AutonomousReportDTO> result = (ResponseEntity<AutonomousReportDTO>)joinPoint.proceed();

            ResponseEntity<AutonomousReportDTO> advisedReport =
     ...
     */
    @Pointcut("execution(* io.github.erp.erp.resources.reports.AutonomousReportResourceProd.getAutonomousReport(..))")
