...
public interface PaymentRepository extends JpaRepository<Payment, Long>, JpaSpecificationExecutor<Payment> {
    ...
    @Query(
        value = "select distinct payment from Payment payment left join fetch payment.paymentCategory left join fetch payment.payee left join fetch payment.settlementCurrencyEntity",
    ...
    @Query("select distinct payment from Payment payment left join fetch payment.paymentCategory left join fetch payment.payee left join fetch payment.settlementCurrencyEntity")
    List<Payment> findAllWithToOneRelationships();

    @Query("select payment from Payment payment left join fetch payment.paymentCategory left join fetch payment.payee left join fetch payment.settlementCurrencyEntity where payment.id =:id")
