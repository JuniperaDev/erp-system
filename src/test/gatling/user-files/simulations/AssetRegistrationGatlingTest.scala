...
class AssetRegistrationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AssetRegistration entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all assetRegistrations")
            .get("/api/asset-registrations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new assetRegistration")
            .post("/api/asset-registrations")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "assetNumber":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created assetRegistration")
                .get("${new_assetRegistration_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created assetRegistration")
            .delete("${new_assetRegistration_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
